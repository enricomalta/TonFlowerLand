document.addEventListener("DOMContentLoaded", function () {
    let walletConnect = false; // Mostra Wallet Modal
    ...
    async function updateUIAndInventory() {
        ...
        try {
            ...
            updateVasoAndPlantImages(walletAddress);
        ...
        userStatus.plantTime.forEach((plant) => {
            ...
            const updateProgress = () => {
                ...
                if (timeRemaining <= 0) {
                    ...
                    // plantPhotoElement.src = `/img/plant/${slotID}.png`; Removi pois já era setado antes
    ...
    window.addEventListener('updateUI', function (e) {
        updateDisplay();
        updateUIAndInventory(); // Chama a nova função
    });
    ...
    // Disconnect Wallet
    btnWalletDisconnect.addEventListener("click", disconnectWallet);
    ...
    async function handleWalletConnect() {
        ...
        updateVasoAndPlantImages(walletAddress);
        try {
            const userData = await updatePlayerStatus(walletAddress); // Aguarda os dados
            if (userData) {
                await updateUIAndInventory(); // Atualiza tudo em um único lugar
            } else {
    ...
    document.querySelectorAll(".playInvetorySlot").forEach(slot => {
        slot.addEventListener("click", async (event) => {
            ...
            if (userConfirmed) {
                await plantSeed(walletAddress, selectedVaseSlot, selectedItem.itemId);
    ...
    function addSlotClickListeners(walletAddress) {
        for (let i = 1; i <= 22; i++) {  // Supondo que você tenha 22 slots
            const slotElement = document.getElementById(`shopSlot${i}`);

            if (slotElement) {
                // Verifica se já existe um listener para evitar múltiplas adições
                if (!slotElement.dataset.listenerAdded) {
                    // console.log(`Adicionando listener para o slot ${i}`);
                    slotElement.addEventListener('click', () => onSlotClick(i, walletAddress));
                    slotElement.dataset.listenerAdded = "true";  // Marca que o listener foi adicionado
                }
            } else {
