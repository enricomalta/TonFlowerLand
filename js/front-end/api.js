...
async function removeItem(walletAddress, itemNome, quantidade) {
...
// Função para plantar
async function plantSeed(walletAddress, selectedVaseSlot, selectedSeed, itemTime, isFertilized, isProtect, isWater) {
...
// Player Request API
async function updatePlayerStatus(walletAddress) {
...
async function checkBalance(item, walletAddress) {
    ...
    try {
        const userStatus = await updatePlayerStatus(walletAddress); // Função que retorna o saldo de tokens do jogador
        ...
        if (parseFloat(tokenBalance) >= parseFloat(itemPrice)) {
            ...
            try {
                ...
                await addItem(walletAddress, itemName, quantidade); // Função para adicionar item ao inventário
...
async function onSlotClick(slotId, walletAddress) {
    if (isProcessing) {
        return;
    }
    ...
    if (!walletAddress) {
        console.error("Erro: walletAddress não foi passado corretamente.");
        isProcessing = false;
        return;
    }
    ...
    await checkBalance(item, walletAddress);

    setTimeout(() => { // Libera cliques após um curto período
        isProcessing = false;
    }, 1000); // Mesmo Delay do compraLimiter(back-end) para evitar múltiplos cliques seguidos(front-end)
}
...
async function fetchProfile(walletAddress) {
    ...
    try {
        const response = await fetch(`${API_URL}/getProfile?walletAddress=${walletAddress}`, {
            method: "GET",
            headers: {
