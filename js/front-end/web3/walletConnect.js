...
async function checkExistingSession() {
    try {
        ...
        if (response.ok) {
            ...
            const sucesso = await UpdateUI(walletAddress);
            if (sucesso) {
                // console.log("Sessão existente restaurada com sucesso.");
            }
...
async function handleWalletConnected(wallet) {
    try {
        ...
        // ✅ CORREÇÃO: Chama /createUser DIRETAMENTE (cria ou faz login + gera JWT)
        console.log('🔐 Autenticando wallet:', walletAddress);
        
        const authResponse = await fetch(`${API_URL}/createUser`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ walletAddress }),
            credentials: 'include' // CRÍTICO: permite cookie HttpOnly
        });

        if (!authResponse.ok) {
            const errorData = await authResponse.json();
            throw new Error(errorData.error || `Erro HTTP ${authResponse.status}`);
        }
        ...
        console.log('JWT armazenado em cookie HttpOnly');

        // Carrega dados do jogador usando a nova função unificada
        const sucesso = await UpdateUI(walletAddress);
        if (sucesso) {
            updateUIForConnectedWallet(walletAddress);
            addSlotClickListeners(walletAddress);
            setupTokenRenewal();
            console.log("UI completamente carregada e atualizada!");
        } else {
            console.error("Erro ao carregar dados do jogador");
        }
    } catch (error) {
        console.error("Erro ao processar wallet conectada:", error);
        alert("Erro ao conectar sua wallet: " + error.message);
    }
}
...
async function initApp() {
    isTelegramWebApp();
    initTranslationSystem(); // Atualiza todos os textos para o idioma atual
    try {
